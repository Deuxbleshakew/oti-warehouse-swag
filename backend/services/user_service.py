...
def update_user(db: Session, *, user_id: int, full_name=None, active=None,
                role_names=None, password=None, catalog_access_mode=None, actor: User,
                source="api") -> User:
    ...
    old = {"full_name": user.full_name, "active": user.active,
          ...
          "catalog_access_mode": user.catalog_access_mode}
    ...
    if catalog_access_mode is not None:
        if catalog_access_mode not in {"all", "restricted"}:
            raise HTTPException(status.HTTP_400_BAD_REQUEST,
                                "Catalog access mode must be all or restricted.")
        user.catalog_access_mode = catalog_access_mode

...
numbers in source files. Provider/network errors are logged but do not reject orders.

## Version 1.0 database upgrade

No manual SQL is required. On startup, the backend creates new tables and adds
required columns to existing SQLite or PostgreSQL databases. This includes the NAV
adjustment queue and NAV item fields. The migration is additive and safe to rerun.

## Confirm version 1.0 is live

After deployment, open `/health` and confirm the build begins with:

    {"status":"ok","build":"1.0"}

The requester login/header and Python admin app should display `1.0`.

...
class ProjectsView(ttk.Frame):
    ...
    def _delete(self):
        ...
        if not project:
            return
        message = (f"Permanently remove {project['name']} from the project list?\n\n"
                   "It will disappear from new orders and permissions. Past orders will keep the original name with a Deleted Project/Event label.")
        if not messagebox.askyesno("Delete project / event", message, parent=self):
            return
        self.spinner.start("Deleting project")
        def work():
            try:

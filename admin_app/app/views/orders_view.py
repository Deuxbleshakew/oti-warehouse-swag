...
class OrdersView(ttk.Frame):
    def __init__(self, parent, api: ApiClient):
        ...
        self._unread_order_ids: set[int] = set()
        self._build()
        self.refresh()
        self._start_polling()
    ...
    def _build(self):
        ...
        self.unread_lbl = ttk.Label(top, text="", style="Badge.TLabel")
        self.unread_lbl.pack(side="left", padx=(0, 8))
        ...
        summary.grid(row=1, column=0, columnspan=2, sticky="ew", padx=12, pady=(0, 8))
        self.summary_vars = {}
        for index, (key, label) in enumerate((
            ("pending", "PENDING"), ("incomplete", "INCOMPLETE"),
            ("approved", "APPROVED"), ("picking", "PICKING"),
            ("fulfilled", "FULFILLED"))):
            summary.columnconfigure(index, weight=1)
            card = ttk.Frame(summary, style="Summary.TFrame", padding=(12, 8))
            card.grid(row=0, column=index, sticky="ew", padx=(0 if index == 0 else 4, 0))
            value = tk.StringVar(value="0")
            self.summary_vars[key] = value
            ttk.Label(card, textvariable=value, style="SummaryNumber.TLabel").pack(anchor="w")
            ttk.Label(card, text=label, style="SummaryLabel.TLabel").pack(anchor="w")

        left = ttk.Frame(self)
        left.grid(row=2, column=0, sticky="nsew", padx=(12, 6), pady=(0, 12))
        ...
        right.grid(row=2, column=1, sticky="nsew", padx=(6, 12), pady=(0, 12))
    ...
    def _set_orders(self, orders):
        ...
        self.orders = orders
        self._known_order_ids.update(int(order["id"]) for order in orders)
    ...
    def _on_select(self, _evt=None):
        ...
        if not selection:
            ...
            return
        oid = int(selection[0])
        self._unread_order_ids.discard(oid)
        self._update_unread_badge()
    ...
    def _open_pick_slip(self, order):
        ...
        for line in lines:
            ...
            if not has_location and not missing_started:
                rows.append("<tr class='section'><td colspan='6'>LOCATION MISSING</td></tr>")
            ...
            image_id = line.get("item_image_id")
            photo = "<span class='no-photo'>NO PHOTO</span>"
            if image_id:
                try:
                    image_src = self.api.item_image_data_uri(image_id)
                    photo = (f"<img src='{html.escape(image_src, quote=True)}' "
                             f"alt='{html.escape(str(line.get('item_name') or ''), quote=True)}'>")
                except (ApiError, OSError):
                    pass
            rows.append(
                "<tr>"
                f"<td class='check'>□</td><td class='thumb'>{photo}</td>"
                f"<td class='loc'>{html.escape(location)}</td>"
...
.check{{font-size:23px;width:38px;text-align:center}}.thumb{{width:76px;text-align:center;padding:4px}}.thumb img{{display:block;width:64px;height:54px;object-fit:contain;margin:auto}}.no-photo{{display:grid;place-items:center;width:64px;height:54px;border:1px dashed #999;font-size:8px;color:#777}}.loc{{font-weight:800;width:115px}}.code{{font-family:monospace;width:112px}}.qty{{font-size:19px;font-weight:800;text-align:center;width:60px}}
.est{{font-size:9px;border:1px solid #111;padding:1px 3px}}.missing{{margin-top:8px;font-size:10px}}
.footer{{margin-top:18px}}.box{{border:2px solid #111;min-height:92px;padding:8px}}
...
<table><thead><tr><th>Pick</th><th>Photo</th><th>Location</th><th>Part #</th><th>Item</th><th>Qty</th></tr></thead><tbody>{''.join(rows)}</tbody></table>
<div class="missing">Items without a saved location are sorted to the bottom under <b>LOCATION MISSING</b>.</div>
<div class="footer"><div class="box"><strong>Picker completion</strong><p>Initials: <span class="line"></span></p><p>Notes:</p></div></div>
</body></html>'''
    ...
    def _start_polling(self):
        def loop():
            while not self._poll_stop.is_set():
                try:
                    is_bootstrap = self._since is None
                    ...
                    if result.get("orders") and not self._poll_stop.is_set():
                        ...
                        self.after(0, lambda rows=rows, notify=not is_bootstrap:
                                   self._handle_order_updates(rows, notify=notify))
                except SessionExpired:
                    return
                except ApiError:
    ...
    def _update_unread_badge(self):
        count = len(self._unread_order_ids)
        self.unread_lbl.configure(text=f" {count} NEW " if count else "")

    def _handle_order_updates(self, rows: list[dict], notify: bool = True):
        if not self.winfo_exists():
            return
        new_orders = ([row for row in rows
                       if row.get("status") == "pending"
                       and int(row["id"]) not in self._known_order_ids]
                      if notify else [])
        for row in rows:
            self._known_order_ids.add(int(row["id"]))
        for row in new_orders:
            self._unread_order_ids.add(int(row["id"]))
            self._show_order_notification(row)
        self._update_unread_badge()
        self.refresh()

    def _show_order_notification(self, order: dict):
        try:
            import winsound
        ...
        ttk.Label(frame, text=f"NEW ORDER #{order['id']}",
                  style="SurfaceHead.TLabel").pack(anchor="w")
        ttk.Label(frame, text=order.get("requester") or "Unknown requester",
                  style="Surface.TLabel").pack(anchor="w", pady=(4, 0))
        ttk.Label(frame, text=order.get("project") or "No project / event",
                  style="SurfaceMuted.TLabel", wraplength=300).pack(anchor="w")
        units = sum(int(line.get("qty_requested") or 0)
                    for line in order.get("lines") or [])
        note = f"{len(order.get('lines') or [])} line(s) · {units} units"
        if order.get("incomplete"):
            note += " · INCOMPLETE"
        ttk.Label(frame, text=note, style="SurfaceMuted.TLabel").pack(anchor="w", pady=(2, 8))
        ...
        def open_order():
            ...
            self.refresh()
            self.after(500, lambda: self._select_order_id(int(order["id"])))
        ttk.Button(frame, text="Open Order", style="Primary.TButton",
                   command=open_order).pack(fill="x")
        win.update_idletasks()
        sw, sh = win.winfo_screenwidth(), win.winfo_screenheight()
        win.geometry(f"+{max(0, sw-win.winfo_reqwidth()-24)}+{max(0, sh-win.winfo_reqheight()-70)}")
        win.after(12000, lambda: win.destroy() if win.winfo_exists() else None)
        fade_in(win)

    def _select_order_id(self, order_id: int):
        iid = str(order_id)

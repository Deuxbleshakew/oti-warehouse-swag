...
class InventoryView(ttk.Frame):
    ...
    def _item_dialog(self, item: dict | None):
        ...
        nav_tracked_var = tk.BooleanVar(value=False if item is None else bool(item.get("nav_tracked")))
        nav_number_var = tk.StringVar(value="" if item is None else item.get("nav_item_number", ""))
        nav_row = ttk.Frame(frm)
        nav_row.grid(row=r, column=0, columnspan=2, sticky="ew", pady=(8, 3))
        nav_check = ttk.Checkbutton(nav_row, text="Tracked in NAV", variable=nav_tracked_var)
        nav_check.pack(side="left")
        ttk.Label(nav_row, text="NAV item number", style="Muted.TLabel").pack(side="left", padx=(16, 6))
        nav_number_entry = ttk.Entry(nav_row, textvariable=nav_number_var, width=20)
        nav_number_entry.pack(side="left")
        def update_nav_state(*_):
            nav_number_entry.configure(state="normal" if nav_tracked_var.get() else "disabled")
        nav_tracked_var.trace_add("write", update_nav_state)
        update_nav_state()
        r += 1
        ...
        def save():
            ...
            if color and not (color.startswith("#") and len(color) in (4, 7)):
                ...
                return
            data["color"] = color
            data["nav_tracked"] = bool(nav_tracked_var.get())
            data["nav_item_number"] = nav_number_var.get().strip() if nav_tracked_var.get() else ""

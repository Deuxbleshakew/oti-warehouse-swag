...
class InventoryView(ttk.Frame):
    ...
    def _build(self):
        ...
        ttk.Checkbutton(top, text="Low stock only", variable=self.low_only,
    ...
    def _item_dialog(self, item: dict | None):
        ...
        if item is None:
            ...
            ttk.Entry(frm, textvariable=qty_var, width=8).grid(
                row=r, column=1, sticky="w", pady=3)
            r += 1
            ttk.Label(frm, text="Photos can be added right after saving — "
                                "reopen the item with Edit.",
        ...
        def save():
            ...
            if item is None:
                try:
                    data["qty_on_hand"] = int(qty_var.get().strip() or 0)
                except ValueError:
                    err_lbl.configure(

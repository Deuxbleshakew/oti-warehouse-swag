...
def apply_theme(root: tk.Misc) -> ttk.Style:
    ...
    style.configure("Head.TLabel", background=PAPER, foreground=INK, font=FONT_HEAD)
    style.configure("SurfaceHead.TLabel", background=SURFACE, foreground=INK,
                    font=FONT_HEAD)
    style.configure("Badge.TLabel", background=SAFETY, foreground=SAFETY_INK,
                    font=("Segoe UI", 8, "bold"), padding=(6, 3))
    style.configure("Summary.TFrame", background=SURFACE, relief="flat")
    style.configure("SummaryNumber.TLabel", background=SURFACE, foreground=SAFETY,
                    font=("Segoe UI", 15, "bold"))
    style.configure("SummaryLabel.TLabel", background=SURFACE, foreground=MUTED,
    ...
    style.configure("TEntry", padding=6, foreground=INK, fieldbackground=SURFACE2)
    style.configure("TCombobox", padding=5, foreground=INK,
                    fieldbackground=SURFACE2, background=SURFACE2, arrowcolor=INK)
